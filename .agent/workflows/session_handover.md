...
7. **引き継ぎメッセージの生成 (Generate Handover Message)**:
   ...
   - 出力フォーマット例:
     ...
     > 次回のAIアシスタントへ：
     > 1. これ以降のすべての対話（返答、確認、思考の出力）および作成ドキュメントは、**例外なくすべて日本語** で記述してください。
     > 2. 英語のドキュメント（SPEC_V1_1.mdなど）を参照した場合でも、ユーザーへの説明は必ず日本語で行ってください。
     > 3. 初期化ドキュメントの読み込みが完了したら、「**初期化完了**」と明確に日本語で宣言し、次のタスクをユーザーに提示して待機してください。
     ```