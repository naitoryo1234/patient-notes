...
### ✅ 実装済み: 記録への画像添付機能（プラグイン方式）
- **要望元**: 特定顧客からの要望（施術前後の写真記録など）
- **実装内容**:
  - `src/plugins/attachments/` にプラグインコード一式
  - 記録カードに「📷」ボタン → モーダルで画像アップロード
  - サムネイル表示、拡大表示、削除機能
  - `NEXT_PUBLIC_ATTACHMENTS_ENABLED=true` で有効化
- **未実装**:
  - プラグインON/OFF設定UI（現在は環境変数で切替）

### 🚀 配布・運用方針 (2025-12-13 決定)

#### 1.デスクトップアプリ化 (Electron)
- **目的**: ITリテラシーが高くない個人事業主でも「アプリ」として直感的に扱えるようにするため。
- **構成案**:
  - Electron + Next.js (Standalone)
  - 内蔵 SQLite
  -インストーラー配布 (exe/dmg)
- **メリット**: ブラウザ操作の誤解（閉じる、戻る等）を防ぎ、OSネイティブな体験を提供。責任境界の明確化。

#### 2. バックアップ・リカバリの強化
- **目的**: ローカル運用の最大リスクである「PC故障・データ紛失」に備える。
- **機能案**:
  - **自動バックアップ**: アプリ終了時や定期的なタイミングでバックアップファイルを特定フォルダ（Google Drive同期フォルダ等）に書き出す。
  - **かんたんリストア**: 初期化状態からバックアップファイルを読み込んで復元する専用UI。
  - **緊急書き出し**: 起動不能時に備えたデータ救出ツール（CLIまたは別exe）。
