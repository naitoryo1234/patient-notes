...
import { getActiveStaff, getAllStaff } from '@/services/staffService';
...
import { getAllSystemSettings } from '@/services/systemSettingService';
...
export default async function RootLayout({
  ...
  const allStaff = await getAllStaff(); // For login screen (includes inactive for operator selection)
  ...
  return (
    <html lang="ja" className="h-full">
      <body className={`${inter.className} h-full overflow-hidden`}>
        <FeaturesProvider dbSettings={dbSettings}>
          <Providers authEnabled={features.auth.enabled} authPin={features.auth.pin}>
            <AuthGuard staffList={allStaff} authEnabled={features.auth.enabled}>
